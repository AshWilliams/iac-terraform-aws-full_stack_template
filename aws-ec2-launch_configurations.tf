resource "aws_launch_configuration" "node" {
  ...
  security_groups       = ["${aws_security_group.vpc-ec2-inst.id}", "${aws_security_group.vpc-ec2-inst-bal_offices_access.id}", "${aws_security_group.vpc-ec2-inst-remote_access.id}"]
