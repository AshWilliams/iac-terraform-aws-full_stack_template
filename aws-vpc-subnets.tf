...
resource "aws_subnet" "pub04" {
  count             = "${var.aws_vpc_sub_pub04_enabled}"
  vpc_id            = "${aws_vpc.public.id}"
  cidr_block        = "${var.aws_vpc_subnet_pub04_cidr_block}"
  availability_zone = "${data.aws_availability_zones.available.names[2]}"
  
  map_public_ip_on_launch = "true"
  
  tags {
    Name          = "VPC-SUB-Pub04-${var.prj_environment}-${var.prj_ecosystem}-${var.prj_application}"
